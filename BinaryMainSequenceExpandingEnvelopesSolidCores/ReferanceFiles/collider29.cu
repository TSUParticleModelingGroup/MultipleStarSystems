/*
nvcc collider29.cu -o collider29 -lglut -lm -lGLU -lGL --use_fast_math  -O3 -run -Xptxas  "-warn-lmem-usage -warn-spills" -arch=sm_35
nvcc collider29.cu -o collider29 -lglut -lm -lGLU -lGL -prec-div=false -prec-sqrt=false -ftz=true -O3 -run
nvcc collider29.cu -o collider29nofast -lglut -lm -lGLU -lGL -O3 -run
*/
...
void setRunParameters()
{
	...
	//Initial velocities of the bodies in kilometers per second:
	...
	//Initial spin direction vectors and magnitude of the bodies:
...
void recordSetupStats()
{
	...
	fprintf(RunStatsFile, "\nThe initial spin in revolutions per time unit of body one: %f", InitialSpin1.w);
	fprintf(RunStatsFile, "\nThe initial spin in revolutions per time unit of body two: %f\n", InitialSpin2.w);
...
void recordContinueConditions(double time)
{
...
static void signalHandler(int signum)
{
	...
	cout << "\n\n******************************************************" << endl;
	cout << "Enter:0 to kill the run." << endl;
	...
	cout << "Enter:7 to change the total run time." << endl;
	cout << "Enter:8 to continue the run." << endl;
	cout << "******************************************************\n\nCommand: ";
...
void runCheckContinue() 
{
	cout << "Enter 0 for a new Run.\nEnter 1 to continue a Run: ";
	cin >> RunCheck;
}
...
{	
	...
	else
	{
		...
		cin >> FolderNameTemp;
		const char* tempName = FolderNameTemp.c_str();
		chdir(tempName);
		...
		cout << "\nThis run ended at " << RunTime*UnitTime/3600.0 << " hours.\n" << endl;
		cout << "Enter how much longer you like it to run: ";
		cin >> AddedRunTime;
		TotalRunTime = AddedRunTime + RunTime;
